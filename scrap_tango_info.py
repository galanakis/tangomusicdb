...
def digest_performance_data(df_input):
    ...
    # - Exclude entries where both the vocalist and the instrumentalists
    #   are empty.
    ind_voc_or_inst = df['Vocalist(s)'].notnull() | df['Instrumentalist(s)'].notnull()

    ind = ind_valid_date & ind_title_has_alpha & ind_voc_or_inst
...
# It loads tango.info performance data in a dataframe.
# It does minimal filtering.
def df_perf_tango_info():
    ...
    df_orig= cat_tables_dataframe(url_spa, url_zxx)
    df = digest_performance_data(df_orig)
